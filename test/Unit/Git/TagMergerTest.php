...
class TagMergerTest extends \PHPUnit_Framework_TestCase {
	...
	public function testFetchTags() {
		...
		$branchReaderMock = $this->mockBuilder->getBranchReaderMock();
		$testee = new Git\TagMerger(
			$tagReaderMock,
			$phpGitMock,
			$eventEmitterMock,
			$branchReaderMock
	...
	public function testMergeBranch() {
		...
		$branchReaderMock = $this->mockBuilder->getBranchReaderMock( [ 'branchExists' ] );
		$branchReaderMock->expects( $this->any() )
			->method( 'branchExists' )
			->willReturn( FALSE );
		$testee = new Git\TagMerger(
			$tagReaderMock,
